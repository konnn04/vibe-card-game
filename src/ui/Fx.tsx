        ...
        {ranked.map((p, i) => {
          ...
          return (
            ...
            >
              ...
              <Avatar name={p.name} preset={i} size={54} className="seat__avatar !h-[54px] !w-[54px] !rounded-[11px]" />
          ...
          <div className="flex gap-3">
            {nextSeats.map((seat, i) => (
              <div key={seat?.id ?? `empty-${i}`} className="flex w-[76px] flex-col items-center gap-1">
                <Avatar name={seat?.name ?? ''} preset={i} size={44} className="seat__avatar !h-11 !w-11 !rounded-[10px]" />
