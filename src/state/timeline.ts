...
/**
 * RÚT NHANH — chồng phạt +N, phạt bắt lỗi, phạt quên hô: số lá BIẾT TRƯỚC,
 * không có gì để cân nhắc nên đổ dồn cho gọn (text.txt: "Draw 6 Cards Fast").
 */
export const DRAW_FAST_MS = 210;
/**
...
function eventMs(e: GameEvent, state: GameState): number {
  switch (e.t) {
    ...
    case 'draw': return Math.max(1, e.cardIds.length) * (e.fast ? DRAW_FAST_MS : DRAW_SLOW_MS);
    case 'skip':
...
export function sfxDelayOf(e: GameEvent): number {
  ...
  if (e.t === 'draw') return e.fast ? DRAW_FAST_MS : DRAW_SLOW_MS;
  return 0;
}
...
export function splitFrames(prev: GameState | null, state: GameState, events: GameEvent[]): Frame[] {
  ...
  const head: Frame = { state: { ...state, turn: prev.turn }, events: events.slice(0, cut) };
  const tail: Frame = { state, events: events.slice(cut) };
...
export function phaseOf(pendingEvents: GameEvent[] | null): TurnPhase {
  ...
  return pendingEvents.some((e) => TURN_END_EVENTS.has(e.t)) ? 'end' : 'start';
}
