...
export function fanTransform(
  ...
  // Quạt của mình: maxSpread 1.55 rad trên R=4.2 -> bề ngang tối đa
  // 2·sin(0.775)·4.2 ≈ 5.9 đơn vị, chiếm khoảng 3/4 bề ngang màn hình ở độ sâu
  // đó (trước là ≈3.6, tức chưa tới một nửa). Cầm 32-40 lá thì mỗi lá hở ra
  // 35-40% bề ngang thay vì 23% — đủ đọc cả chỉ số góc lẫn màu.
  // perCard chia lại theo R mới (÷1.47) để tay ÍT bài vẫn xoè y như cũ.
  const maxSpread = self ? 1.55 : 1.0;
  /**
   * MỘT hằng số, không chia bậc theo số lá.
   *
   * Bản cũ hạ perCard theo bậc (>8 lá, >12 lá) để tay đông khỏi tràn. Hệ quả là
   * quạt CO LẠI đúng lúc rút thêm bài: từ 12 sang 13 lá, bậc mới kéo bề ngang
   * từ 3.0 xuống 2.6 — rút thêm một lá mà chỗ để nhìn lại hẹp đi. Giờ để nó
   * tăng đều rồi cho maxSpread chặn trên: quạt chỉ nở ra, không bao giờ co lại,
   * và từ ~18 lá trở đi thì chạm trần, phần hở thu hẹp dần một cách đều đặn.
   */
  const perCard = self ? 0.092 : 0.11;
  const minSpread = self ? 0.24 : 0.35;
  const spread = count <= 1 ? 0 : Math.min(maxSpread, Math.max(minSpread, perCard * (count - 1)));
  ...
  // Dạt 2 bên khi có 1 lá khác trong tay đang hover — lá càng gần lá đang xem
  // càng dạt xa, giảm dần theo khoảng cách (mô phỏng cầm bài thật, rút 1 lá ra
  // xem thì các lá cạnh nó phải giãn ra nhường chỗ). Biên độ nhỏ, chỉ đủ hé ra
  // chứ không xô lệch cả bàn tay.
  if (self && !opts.isHovered && opts.hoverProgress !== undefined) {
    const tHover = opts.hoverProgress * spread;
    const d = t - tHover;
    const dist = Math.abs(d);
    // Cũng là số đo bằng GÓC -> chia lại theo R mới, giữ nguyên biên độ dạt
    // THẬT trên mặt bàn.
    const REPEL_RADIUS = 0.29;
    const REPEL_MAX = 0.048;
