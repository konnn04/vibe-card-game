...
import { serverNow, startClockSync, setLocalClock } from './clock';
 import {
  catchUpScale, DEAL_STAGGER_MS, DRAW_FAST_STAGGER_MS, DRAW_SLOW_MS, frameMs, MIN_PHASE_MS, phaseOf, sfxDelayOf, splitFrames,
 ...
 *
 * KHÔNG dùng JSON.stringify cả event: bản server
...
function normalizeClientGame(g: GameState): GameState {
  return {
    ...
    // Đảm bảo wildColors luôn là object để Cards.tsx đọc thẳng không cần check.
...
export const useMatch = create<MatchStore>((set, get) => ({
  ...
  /** Chuyển sang chế độ online: state đến từ Socket.IO server, mọi action gửi lên server. */
  ...
  act(action) {
    ...
    if (mode === 'online') {
      ...
      // Socket.IO -> server -> broadcast (qua proxy Discord có thể mất
      // một khoảng ngắn). Đó chính là lỗi "đánh ra rồi mà không biết bài ra chưa":
      ...
      const rawSnapshot = authoritative(get);
      const snapshot = rawSnapshot ? mergeHand(rawSnapshot, myId, get().myCards) : null;
      ...
      if (canPredict && snapshot) {
        ...
        if (!bad) {
          ...
          enqueue(get, set, guess, guessEvents, true);
        }
      }
 ...
 */
function enqueue(get: Getter, set: Setter, state: GameState, events: GameEvent[], isOptimistic = false) {
  ...
  for (const f of splitFrames(prev, state, events)) {
    if (isOptimistic) f.isOptimistic = true;
    frames.push(f);
  }
  pump(get, set);
}
...
function pump(get: Getter, set: Setter) {
  ...
  while (frames.length > 0) {
    ...
    for (const e of frame.events) {
      ...
      if (!frame.isOptimistic && sig && takeEcho(sig)) { echoed.add(e); continue; }
      ...
      // RÚT NHIỀU LÁ (phạt dồn, +2, +4, stack): các lá bay so le từng lá (stagger).
      // Rải SFX cho TỪNG LÁ bay đến tay để khớp với hình ảnh rút từng lá liên thanh.
      if (e.t === 'draw' && e.cardIds.length > 1) {
        const stagger = e.fast ? DRAW_FAST_STAGGER_MS : DRAW_SLOW_MS;
        const n = e.cardIds.length;
        for (let i = 0; i < n; i++) {
          sfxTimers.push(
            setTimeout(() => {
              if (e.penalty && i === 0) {
                // Tiếng penalty trầm cho phát đầu
                playSfx('penalty', Math.max(0.5, 1 - Math.min(n, 10) * 0.055));
              }
              // Tiếng vút/đáp của từng lá bài
              playSfx(e.fast ? 'whoosh' : 'draw', 1 + (i % 4) * 0.06);
            }, i * stagger + (e.fast ? 40 : 80))
          );
        }
        continue;
      }
