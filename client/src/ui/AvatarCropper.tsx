...
import { AVATAR_KEY, encodeSquareWebp, putBlob, sharedAvatarDataUrl } from '@/src/lib/idb';
...
export function ImageCropperModal({ src, onClose }: { src: string; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    if (mode === 'online' && code) {
      void sharedAvatarDataUrl().then((img) => {
        if (img) void api.avatar(code, meId, loadToken(code), img);
      });
    }
