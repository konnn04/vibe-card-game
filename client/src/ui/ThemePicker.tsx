...
export function ThemePicker({
  ...
  return (
    <div className={`flex flex-wrap gap-2.5 ${disabled ? 'opacity-70' : ''}`}>
      {THEMES.map((th) => {
        ...
        return (
          <button
            ...
            className={`display flex items-center gap-2.5 rounded-xl text-[#EDE0FA] ${compact ? 'px-2.5 py-1.5 text-[15px]' : 'px-3 py-2 text-[17px]'} ${disabled ? 'cursor-not-allowed' : ''}`}
            style={{
